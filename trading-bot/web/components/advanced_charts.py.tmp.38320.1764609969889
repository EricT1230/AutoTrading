...
class AdvancedChartComponent:
    ...
    def render_chart_controls(self):
        ...
        with indicator_col1:
            show_ma = st.checkbox("移動平均線 (MA)", value=True, key="show_ma")
        ...
        with indicator_col4:
            show_bollinger = st.checkbox("布林帶", value=False, key="show_bollinger")
    ...
    def render_realtime_chart(self):
        ...
        with col1:
            st.info(f"📈 正在顯示 {symbol} {timeframe} 圖表，包含 {', '.join(indicators)} 指標")
        
        with col2:
            auto_update = st.checkbox("圖表自動更新", value=True, key="chart_auto_update")
        
        # 創建圖表容器
        chart_container = st.empty()
        
        # 生成圖表
        with st.spinner("🔄 正在生成專業圖表..."):
        ...
        if fig:
            ...
            try:
                ...
                if kline_data is not None and not kline_data.empty:
                    ...
                    with metric_col1:
                        st.metric("最新價格", f"${latest['close']:,.4f}")
                    
                    with metric_col2:
                        st.metric("24h變化", f"{change:+.2f}%")
        ...
        if auto_update:
            ...
            if current_time - st.session_state.chart_last_update >= 10:  # 每10秒刷新圖表
