...
export const Dashboard: React.FC = () => {
    ...
    // 可用的時間框架選項
    const timeframes = [
        { value: '1m', label: '1分鐘', interval: 60 },
        { value: '5m', label: '5分鐘', interval: 300 },
        { value: '15m', label: '15分鐘', interval: 900 },
        { value: '1h', label: '1小時', interval: 3600 },
        { value: '4h', label: '4小時', interval: 14400 },
    ...
    const handleTimeframeChange = async (timeframe: string) => {
        ...
        try {
            ...
            if (result.success) {
                addLog(`已切換到 ${timeframe} 實時數據`, 'success');
            }
        } catch (error) {
    ...
    useEffect(() => {
        const initializeData = async () => {
            ...
            try {
                ...
                const result = await response.json();
                if (result.success) {
                    addLog(`已訂閱 ${selectedTimeframe} 實時數據推送`, 'success');
                ...
                if (data.success && data.data && Object.keys(data.data).length > 0) {
                    ...
                    if (data.system_status) {
                        ...
                        if (systemStatus.running) {
                            setStatus({ active: true, strategy: 'ICT NY FVG' });
                            addLog(`實時更新 #${systemStatus.update_count}: $${Math.round(currentPrice)}`, 'success');
                        }
                    }
                }
            } catch (error) {
    ...
    useEffect(() => {
        if (lastMessage !== null) {
            try {
                ...
                if (event.type === 'CONNECTION_SUCCESS') {
                ...
                } else if (event.type === 'KLINE_UPDATE') {
                    const k = event.data;
                    const newKline = {
                        time: k.time, // Already in seconds from backend
                        open: k.open,
                        high: k.high,
                        low: k.low,
                        close: k.close,
                    ...
                    setKlines(prev => {
                        ...
                        if (newKline.time > last.time) {
                            // New candle
                            const updated = [...prev.slice(-99), newKline];
                            return updated;
                        } else if (newKline.time === last.time) {
                    ...
                    addLog(`K線更新: ${k.symbol} - 價格: ${k.close.toFixed(2)}`, 'info');

                } else if (event.type === 'TICKER_UPDATE') {
                    const ticker = event.data;
                    setCurrentPrice(ticker.price);
                    setPriceChange(ticker.change_percent_24h);
                    addLog(`價格更新: ${ticker.symbol} - ${ticker.price.toFixed(2)} (${ticker.change_percent_24h.toFixed(2)}%)`, 'info');
                
                } else if (event.type === 'SIGNAL_GENERATED') {
                    addLog(`交易信號: ${event.data.direction} @ ${event.data.price}`, 'success');
                } else if (event.type === 'ORDER_FILLED') {
                    addLog(`訂單成交: ${event.data.side} ${event.data.amount}`, 'success');
                }

            } catch (e) {
                console.error("Failed to parse message", e);
                addLog(`消息解析失敗: ${e}`, 'error');
            }
        }
    }, [lastMessage]);
            ...
            <header className="flex justify-between items-center mb-8">
                ...
                <div className="flex items-center gap-4">
                    <div className={clsx("flex items-center gap-2 px-3 py-1 rounded-full text-sm font-medium",
                        readyState === ReadyState.OPEN ? "bg-green-500/10 text-green-500" : "bg-red-500/10 text-red-500"
                    )}>
                        <div className={clsx("w-2 h-2 rounded-full", readyState === ReadyState.OPEN ? "bg-green-500" : "bg-red-500")} />
                        {connectionStatus}
                    </div>
                </div>
            </header>
                        ...
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <div className="flex flex-col gap-2">
                                ...
                                <div className="flex gap-2 justify-end">
                                    <span className="px-2 py-1 bg-blue-500/20 text-blue-500 rounded text-xs">實時</span>
                                    {lastUpdateTime && (
